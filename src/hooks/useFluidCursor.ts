...
const useFluidCursor = () => {
  ...
  // RED/BLACK THEME: Only generate red-ish hues
  function generateColor() {
    // Hue range: 0.0 to 0.05 (red) and 0.95 to 1.0 (also red)
    const hueRand = Math.random();
    const h = hueRand < 0.5 ? hueRand * 0.08 : 0.95 + hueRand * 0.05;
    const s = 0.7 + Math.random() * 0.3; // 0.7-1.0 saturation
    const v = 0.8 + Math.random() * 0.2; // 0.8-1.0 brightness
